export const POPUP_FADE_IN_MS = 500
export const POPUP_FADE_OUT_MS = 500
export const POPUP_SHOW_MS = 1500
export const UNDERLINE_CHECK_INTERVAL_MS = 1000

export const PRICE_UPDATE_INTERVAL_MS = 10_000
export const PRICE_PRECISION_DIGITS = 8
export const PRICE_PRECISION = BigInt(10) ** BigInt(PRICE_PRECISION_DIGITS)

export const MAX_ALLOWED_BUY_DURATION_MS = 60 * 60 * 1 // one hour
export const MAX_KUCOCOIN_DECIMALS_DISPLAY = 8

// dex
export const DEX_FACTOR_BIPS = BigInt(997)
export const DEX_MAX_BIPS = BigInt(10_000)