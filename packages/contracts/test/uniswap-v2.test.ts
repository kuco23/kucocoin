...
import type { ERC20, FakeERC20, FakeWNat, UniswapV2Router, UniswapV2Factory, UniswapV2Pair } from '../types'
...
describe("UniswapV2", () => {
  ...
  async function addLiquidity(
    ...
    await tokenA.connect(signer).approve(uniswapV2Router, amountA)
    await tokenB.connect(signer).approve(uniswapV2Router, amountB)
    await uniswapV2Router.connect(signer).addLiquidity(
      tokenA, tokenB, amountA, amountB,
      0, 0, signer, ethers.MaxUint256)
  ...
  beforeEach(async () => {
    ...
    uniswapV2Router = await factories.uniswapV2Router.connect(admin).deploy(uniswapV2Factory, wrappedNativeToken)
  ...
  it("should remove liquidity with tokenA / tokenB pair", async () => {
    ...
    await pairAB.connect(provider).approve(uniswapV2Router, liquidity)
    await uniswapV2Router.connect(provider).removeLiquidity(
  ...
  it("should use tokenA / wrappedNativeToken pair when using addLiquidityNat", async () => {
    ...
    await tokenA.connect(admin).approve(uniswapV2Router, amountTokenA)
    await uniswapV2Router.connect(admin).addLiquidityETH(
      tokenA, amountTokenA, amountTokenA, amountNative, admin, ethers.MaxUint256,
      { value: amountNative }
    )
    const pair = await getPairFor(tokenA, wrappedNativeToken)
