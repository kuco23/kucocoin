...
describe("KucoCoin", () => {
  ...
  describe("dex integration", async () => {
    ...
    it("should test buying kucocoin", async () => {
      ...
      await kucocoin.connect(admin).buy(receiver, { value: amountNat })
    ...
    it("should test selling kucocoin", async () => {
      ...
      const estimatedGasUsed = await kucocoin.sell.estimateGas(amountKUCO, receiver)
      await kucocoin.connect(admin).sell(amountKUCO, receiver)
